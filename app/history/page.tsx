...
import { Trash2, ChevronDown, ChevronUp, BookOpen, Clock, Timer } from 'lucide-react'
    ...
    <div className="space-y-5">
      ...
      {loading && (
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => <div key={i} className="h-24 rounded-xl bg-zinc-800 animate-pulse" />)}
        </div>
      )}

      {!loading && workouts.length === 0 && (
        <div className="text-center py-16 text-zinc-500"><p>No workouts logged yet.</p></div>
      )}
      ...
      <div className="space-y-3">
        {workouts.map((w) => {
          ...
          const allExercises = groups.flatMap((g) => Object.keys(g.exercises))
          ...
          return (
            <div key={w.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-4 py-4 cursor-pointer" onClick={() => toggle(w.id)}>
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    ...
                    {w.duration_seconds && w.duration_seconds > 0 && (
                      <span className="flex items-center gap-1 text-orange-400 text-xs">
                        <Timer size={11} /> {formatDuration(w.duration_seconds)}
                      </span>
                    )}
                  </div>
                  <p className="text-zinc-400 text-sm mt-1">
                    {allExercises.length} exercise{allExercises.length !== 1 ? 's' : ''} · {totalSets} sets
                    {totalVolume > 0 && ` · ${totalVolume.toLocaleString()} kg`}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <button onClick={(e) => { e.stopPropagation(); deleteWorkout(w.id) }} className="text-zinc-600 hover:text-red-400 transition-colors">
                    <Trash2 size={16} />
                  </button>
              ...
              {isOpen && (
                <div className="px-4 pb-4 border-t border-zinc-800 pt-3 space-y-3">
                  {groups.map(({ routineName, exercises }) => {
                    const content = (
                      <div className="space-y-3">
                        {Object.entries(exercises).map(([name, sets]) => (
                          <div key={name}>
                            <p className={`font-medium text-sm mb-1.5 ${routineName ? 'text-zinc-200' : 'text-orange-400'}`}>{name}</p>
                    ...
                    return routineName ? (
                      <div key={routineName} className="rounded-xl border border-orange-500/25 bg-orange-500/5 p-3 space-y-3">
                    ...
                    ) : <div key="__solo__">{content}</div>
                  })}

                  {w.notes && <p className="text-zinc-500 text-sm italic border-t border-zinc-800 pt-3">{w.notes}</p>}
                </div>
              )}
            </div>
          )
